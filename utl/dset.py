...
def getdatainfo(data_dir):
    ...
    tgt_dir = os.path.join(data_dir, 'Target')

    src_fns = glob(os.path.join(src_dir, '*.nii'))
    tgt_fns = glob(os.path.join(tgt_dir, '*.nii'))
    assert len(tgt_fns) == len(src_fns) and len(src_fns) != 0
    
    # if len(CT_fns) != len(STR_fns) or len(CT_fns) == 0:
        # raise ValueError(f'Number of source and target images must be equal and non-zero')
    datainfolist=[]
    for i,c in enumerate(src_fns):
        # Example naming of data, but change as you desire
        # source_001.nii
        # target_001.nii

        tgt_add=(src_fns.replace('source','target'))

        datainfolist.append(DataInfoTuple(src_fns,tgt_add))
    return datainfolist
...
def getsample(data_tup):
    ...
    inp = inp[np.newaxis, ...]  # add channel axis
...
def getData(filenames):
    ...
    src_fns=filenames.sourceaddress
    tgt_fns=filenames.targetaddress

    inP=nib.load(src_fns).get_fdata().astype(np.float32)
    
    outP=nib.load(tgt_fns).get_fdata().astype(np.float32)
