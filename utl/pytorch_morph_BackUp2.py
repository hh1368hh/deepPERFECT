...
def dilate_pytorch(image, strel, origin=(0, 0), border_value=0, additive=False):
    ...
    image_pad=image_pad.squeeze(0).squeeze(0)
    ...
    # print(sums.shape)
        ...
        sums = image_unfold[:,strel_flatten.squeeze(0).squeeze(1),:]
...
def erode_pytorch(image, strel, origin=(0, 0), border_value=0, additive=False):
    ...
    if additive:
        ...
        rest = image_unfold - strel_flatten
    else:
        rest = image_unfold[:,strel_flatten.squeeze(0).squeeze(1),:]
    # Take maximum over the neighborhood
    result, _ = rest.min(dim=1)
    # Reshape the image to recover initial shape
    return torch.reshape(result, image.shape)
