...
class HH_Unet3(nn.Module):
    def __init__(self, in_channel=1, conv_channel=8, out_channel=1, kernel_size=3, up_mode='upconv'):
        ...
        # 4th Layer
        self.en_block4 = EncodeBlock(conv_channel*4, conv_channel*8, conv_channel*8, kernel_size=kernel_size)        
        
        ## Bridge
        self.br_block3 = BridgeBlock(conv_channel*8, conv_channel*16, conv_channel*8, kernel_size=kernel_size)
        ...
        # 4th Layer
        self.de_block4 = DecodeBlock(conv_channel*16, conv_channel*8, conv_channel*4, kernel_size= kernel_size, up_mode=up_mode)
    ...
    def forward(self, input_batch):
        ...
        en4_block_out = self.en_block4(en3_block_out)
        # print(en4_block_out.size())

        # Bridge

        br_block_out = self.br_block3(en4_block_out)
        # print(br_block_out.size())
        
        # Decoding Path

        de4_block_out=self.de_block4(self.crop_and_concat(br_block_out, en4_block_out, crop=True))
        # print(de4_block_out.size())
        de3_block_out=self.de_block3(self.crop_and_concat(de4_block_out, en3_block_out, crop=True))
